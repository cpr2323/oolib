    ...
    bool compareChildrenAndThierPropertiesUnordered (juce::ValueTree firstVT, juce::ValueTree secondVT, LogCompareFailures logCompareFailures, StopAtFirstFailure stopAtFirstFailure)
    {
        ...
        if (firstVT.getNumChildren () != secondVT.getNumChildren ())
        {
             if (logCompareFailures == LogCompareFailures::yes)
                 DebugLog ("compareChildrenAndThierPropertiesUnordered number of children differ - first: " + juce::String (firstVT.getNumChildren ()) + ", second: " + juce::String (secondVT.getNumChildren ()));
            jassertfalse;
            return false;
        }
        auto childIndex { 0 };
        while (firstVtIterator != firstVT.end ())
        {
            ...
            {
                 if (logCompareFailures == LogCompareFailures::yes)
                     DebugLog ("compareChildrenAndThierPropertiesUnordered/comparePropertiesUnOrdered failed. child index: " + juce::String (childIndex));
                jassertfalse;
                return false;
            }
            ++firstVtIterator;
